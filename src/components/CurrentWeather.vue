...
<template>
  ...
  <div class="flex flex-col items-center gap-y-4 mt-10">
    ...
    <img v-if="currentWeather" :src="`https://openweathermap.org/img/wn/03d@2x.png`" class="w-40 h-40">
    <!-- <img v-else :src="`https://openweathermap.org/img/wn/${currentWeather.weather[0].icon}@2x.png`" class="w-40 h-40"> -->
    <div v-if="currentWeather" class="flex flex-col items-center">
      <p class="text-6xl font-bold text-white">{{ currentWeather.temp }}°F</p>
